...
def main():
    ...
    #transform
    test_transform_set = []
    test_transform_set.append(None) # for None aug ---1
    test_transform_set.append(None) # for permute---2

    #aug 90---3
    logger.info('augmentation rotate')
    logger.info('rotate_angle:{}'.format(90))
    test_transform = transform.RandomRotate_test(rotate_angle=90, along_z=args.get('rotate_along_z', True))
    test_transform_set.append(test_transform)

    #aug 180---4
    logger.info('augmentation rotate')
    logger.info('rotate_angle:{}'.format(180))
    test_transform = transform.RandomRotate_test(rotate_angle=180, along_z=args.get('rotate_along_z', True))
    test_transform_set.append(test_transform)

    #aug 270---5
    logger.info('augmentation rotate')
    logger.info('rotate_angle:{}'.format(270))
    test_transform = transform.RandomRotate_test(rotate_angle=270, along_z=args.get('rotate_along_z', True))
    test_transform_set.append(test_transform)

    if args.data_name == 's3dis':
        #shift + 0.2---6
        logger.info('augmentation shift')
        logger.info('shift_range:{}'.format(0.2))
