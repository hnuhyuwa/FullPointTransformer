...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/s3dis/s3dis_fptransformer_repro.yaml', help='config file')
    parser.add_argument('opts', help='see config/s3dis/s3dis_fptransformer_repro.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'fptransformer_seg_repro':
        from model.fptransformer.fptransformer_seg import fullpointtransformer_seg_repro as Model
    else:
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr = 0.006, weight_decay=0.05)
    ...
    if args.distributed:
        ...
        if main_process():
            logger.info('classifier paramters:{}'.format(sum([x.nelement() for x in model.parameters()])))
