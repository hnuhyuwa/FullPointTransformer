...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    ...
    grouped_xyz -= new_xyz.unsqueeze(1) # (m, nsample, 3)# xyz_different
...
def queryandgroup1(nsample, xyz, new_xyz, feat, idx, offset, new_offset, g_r, use_xyz=True):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b), g_r:(n, c)
    output: new_feat: (m, c+3, nsample), grouped_idx: (m, nsample)
    """
    assert xyz.is_contiguous() and new_xyz.is_contiguous() and feat.is_contiguous()
    if idx is None:
    ...
    if g_r is None:
        ...
        c = g_r.shape[-1]
        xyz = torch.cat((xyz, g_r),dim = -1) #(n, 3+c)
        grouped_xyz = xyz[idx.view(-1).long(), :].view(m, nsample, 3 + c) # (m, nsample, 3+c)


    grouped_xyz -= xyz.unsqueeze(1) # (m, nsample, 3)  /    (m, nsample, 3+c)
    grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c) # (m, nsample, c)

    if use_xyz:
        return torch.cat((grouped_xyz, grouped_feat), -1) # (m, nsample, 3+c)    /    (m, nsample, 3 + c + c)
    else:
        return grouped_feat
...
def queryandgroup3(nsample, xyz, new_xyz, feat, idx, offset, new_offset, g_r, new_g_r, use_xyz=True):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b)
    output: new_feat: (m, c+3, nsample), grouped_idx: (m, nsample)
    """
    ...
    n, m, c = xyz.shape[0], new_xyz.shape[0], feat.shape[1]
    xyz = torch.cat((xyz, g_r),dim = -1)
    grouped_xyz = xyz[idx.view(-1).long(), :].view(m, nsample, 3+c) # (m, nsample, 3+c)

    #grouped_xyz = grouping(xyz, idx) # (m, nsample, 3)
    new_xyz = torch.cat((new_xyz, new_g_r),dim = -1)
    grouped_xyz -= new_xyz.unsqueeze(1) # (m, nsample, 3+c)# xyz_different

    grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c) # (m, nsample, c)
    #grouped_feat = grouping(feat, idx) # (m, nsample, c)

    if use_xyz:
        return torch.cat((grouped_xyz, grouped_feat), -1) # (m, nsample, 3+c+c)
    else:
        return grouped_feat